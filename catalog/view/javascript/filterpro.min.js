...
function success(g, b, append) {
    ...
    // без выбора способов отображения
    $(".product-grid, .product-list").html('');
    $(".product-grid, .product-list").append(g.result_html);
    ...
    if (g.totals_data) {
        ...
        $('.a_name').each(function (k, v) {
            ...
            if (atts[at_v_i]) {
                ...
                $(v).removeAttr("disabled").css({color: '#5E5E5E'});
                if(hide_options && v.tagName != "OPTION")
                    $(v).parent().parent().show();
            } else {
                $('[at_v_t="' + at_v_i_e + '"]').text($('[at_v_t="' + at_v_i_e + '"]').attr('data-value'));
                $(v).attr("disabled", "disabled").css({color: '#D5D5D5'});
                if(hide_options && v.tagName != "OPTION")
        ...
        $.each(g.totals_data.manufacturers, function (f, k) {
            if (k.id) {
                ...
                j.removeAttr("disabled").css({color: '#5E5E5E'});
        ...
        $(".manufacturer_value").each(function (f, k) {
            ...
            if ($.inArray(l[1], h) < 0) {
                j.attr("disabled", "disabled").css({color: '#D5D5D5'});
                if(hide_options && j[0].tagName != "OPTION")
        ...
        $.each(g.totals_data.options, function (j, k) {
            if (k.id) {
                ...
                if (f.exists()) {
                    f.removeAttr("disabled").css({color: '#5E5E5E'});
        ...
        $(".option_value").each(function (j, k) {
            ...
            if ($.inArray(l[1], e) < 0) {
                f.attr("disabled", "disabled").css({color: '#D5D5D5'});
                if(hide_options && f[0].tagName != "OPTION")
...
function doFilter(getPrice, getTotals, append) {
    ...
    if (cache[h]){
        ...
        $.ajax({
            ...
            type:"POST",
            data:a + (getPrice ? "&getPriceLimits=true" : "") + (getTotals ? "&getTotals=1" : ""), dataType:"json",
            success:function (g) {
