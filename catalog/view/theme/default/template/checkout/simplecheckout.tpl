...
<?php } ?>
    <div class="simplecheckout" id="simplecheckout_form">
      ...
      <?php } ?>
      <?php if ($simple_steps) { ?>
        <div class="simplecheckout-button-block buttons actions" id="step_buttons">
            <div class="simplecheckout-button-right">
                <a class="button btn btn-style" onclick="simplecheckout_next();" id="simplecheckout_next"><span><?php echo $button_next; ?></span></a>
            </div>
            <div class="simplecheckout-button-left">
                <a class="button btn btn-style" onclick="simplecheckout_prev()" id="simplecheckout_prev"><span><?php echo $button_prev; ?></span></a>
            </div>
        </div>
      <?php } ?>
        <div class="simplecheckout-button-block buttons actions" id="buttons" <?php if ($block_order) { ?>style="display:none;"<?php } ?>>
            <div class="simplecheckout-button-right">
              <?php if ($simple_common_view_agreement_checkbox) { ?><label><input type="checkbox" id="agree" name="agree" value="1" <?php if ($agree == 1) { ?>checked="checked"<?php } ?> /><?php echo $text_agree; ?></label>&nbsp;<?php } ?><a class="button btn btn-style btn-order" onclick="simplecheckout_submit();" id="simplecheckout_button_confirm"><?php echo $button_order; ?></a>
            </div>
          <?php if ($simple_show_back) { ?>
            <div class="simplecheckout-button-left">
                <a class="button btn btn-style" onclick="history.go(back_count)"><span><?php echo $button_back; ?></span></a>
            </div>
          <?php } ?>
        </div>
    </div>
