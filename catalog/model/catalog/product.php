...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'unit_count'     => $query->row['unit_count'],
				'meters_package'     => $query->row['meters_package'],
    ...
    if(!empty($data['latest_mark'])){
      $sql .= " LEFT JOIN " . DB_PREFIX . "product_status pss ON (p.product_id = pss.product_id) LEFT JOIN " . DB_PREFIX . "status ss ON (pss.status_id = ss.status_id) ";
    }
    ...
    $sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ";

    if(!empty($data['latest_mark'])){
      $sql .= " AND ss.".$data['latest_mark_row']." = '" .$data['latest_mark']. "' ";
    }
